...
async function getPatientUploadedFiles(req, res, next) {
  try {
    ...
    const fetchStorageObjects = async (pathPrefix, label) => {
      const safePrefix = String(pathPrefix || "").trim();
      if (!safePrefix) {
        return [];
      }

      const apiUrl = `${supabaseUrl}/rest/v1/storage.objects?bucket_id=eq.${encodeURIComponent(bucketName)}&name=like.${encodeURIComponent(`${safePrefix}%`)}&order=created_at.desc&limit=200`;

      console.log(
        `[getPatientUploadedFiles] Calling storage.objects (${label}):`,
        apiUrl.replace(supabaseKey, "***"),
      );
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error(
          `[getPatientUploadedFiles] storage.objects REST API error (${label}):`,
          response.status,
          errorText,
        );
        return [];
      ...
      try {
        ...
        console.warn(
          `[getPatientUploadedFiles] storage.objects response was not valid JSON (${label}):`,
          jsonError.message,
        );
        return [];
      }
    ...
    if (!filesToReturn.length) {
      const storagePrefixes = [];

      if (normalizedEnccode) {
        storagePrefixes.push(`lab-results/${normalizedEnccode}`);
      }

      storagePrefixes.push(`lab-results/${hpercode}`);

      const storageObjects = [];
      for (const prefix of storagePrefixes) {
        const objects = await fetchStorageObjects(prefix, prefix);
        for (const item of objects) {
          const objectKey = String(item?.name || "").trim();
          if (!objectKey || seenKeys.has(objectKey)) {
            continue;
          }
      ...
      if (storageObjects.length > 0) {
        const storageClientForFallback = getSupabaseStorageClient();
        const synthesizeRows = async (objectRow) => {
          const objectName = String(objectRow?.name || "").trim();
          const fileName = objectName.split("/").pop() || objectName || "lab_result.pdf";
          const fallbackPath = objectName.startsWith("lab-results/")
            ? objectName
            : `lab-results/${objectName}`;

          let fileUrl = "";
          if (storageClientForFallback) {
            fileUrl =
              (await resolveUploadedFileUrl(
                storageClientForFallback,
                bucketName,
                {
                  ...objectRow,
          ...
          if (!fileUrl) {
            fileUrl = `${supabaseUrl}/storage/v1/object/public/${bucketName}/${objectName}`;
          }

          return {
            id: objectRow?.id || fallbackPath,
            hpercode,
            enccode: enccode || null,
            orcode: null,
            proccode: null,
            ...
            storage_path: fallbackPath,
            file_size: objectRow?.metadata?.size || null,
            content_type: objectRow?.metadata?.mimetype || "application/pdf",
            uploaded_by: null,
            remarks: null,
        ...
        filesToReturn = await Promise.all(storageObjects.map(synthesizeRows));
      }
    }
